...
var SheetSettingsTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian2.Setting(containerEl).setName("Native table post processing").setDesc("Enable this setting to use Obsidian Sheets' renderer ").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.nativeProcessing).onChange(async (value) => {
        ...
        (_b = this.app.workspace.getActiveViewOfType(import_obsidian2.MarkdownView)) == null ? void 0 : _b.previewMode.rerender(true);
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Use paragraphs in cells").setDesc("Enable this setting to use paragraphs for table cells ").addToggle(
...
var META_DELIMETER = "---";
var SheetElement = class extends import_obsidian3.MarkdownRenderChild {
  constructor(el, source, ctx, app, plugin) {
    super(el);
    this.el = el;
    this.source = source;
    ...
    this.registerMarkdownPostProcessor(async (el, ctx) => {
      ...
      for (const tableEl of Array.from(tableEls)) {
        ...
        if (!sec) {
          ...
          source = (0, import_obsidian4.htmlToMarkdown)(tableEl).trim().replace(/\\\\/g, "$&$&");
          if (!source)
            return;
        } else {
